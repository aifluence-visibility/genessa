-- Stage 1 · approve-only RPC (atomic: update approval_requests + insert activity_events)
-- Internal / service_role only. Extend later for reject + rerun.
...
comment on function public.admin_approve_approval_request(uuid, text) is
  'Atomically approves an open approval_request and appends activity_events (Stage 1 internal).';
